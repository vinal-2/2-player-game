...
export class PingPongController implements GameController {
  private model: PingPongModel;
  private mode: "friend" | "bot";

  constructor(model: PingPongModel, mode: "friend" | "bot") {
    this.model = model;
    this.mode = mode;
  }
  ...
  public handleInput(input: { type: string; playerId?: "player1" | "player2"; y?: number }): void {
    switch (input.type) {
      case "paddleMove":
        if (input.playerId && input.y !== undefined) {
          this.model.movePaddle(input.playerId, input.y);
        }
        break;
      case "reset":
        this.reset();
  ...
  public update(): void {
    ...
    if (this.mode === "bot") {
      // Basic bot logic
      if (Math.abs(this.model.state.ball.ySpeed) > 0) {
        this.model.movePaddle(
          "player2",
          this.model.state.ball.y - this.model.state.player2.height / 2
        );
      }
    }
  }
